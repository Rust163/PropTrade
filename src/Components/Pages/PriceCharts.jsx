...
function PriceCharts() {
  ...
  const fetchCandleData = async (security, interval = selectedInterval) => {
    ...
    try {
      ...
      const candles = response.candles.data
        .map((item) => {
  ...
  const handleOpenInNewWindow = (stock) => {
    const url = `/chart/${stock.SBER}`
  ...
  const tradeModes = ['Все режимы', 'TQBR', 'TQTF', 'SPEQ', 'TQIF', 'SMAL']

  const listLevel = ['Все эшелоны', '1 эшелон', '2 эшелон', '3 эшелон']
  ...
  if (loading)
    return (
      <div className="loading loading-stocks">
        Загружаем рынки ценных бумаг...
      </div>
    )
            ...
            <div className="pt-3">
              <ul className="nav flex-column">
                <li className="nav-item">
                  <a
                    className="nav-link active link-secondary ff-text"
                    aria-current="page"
                    href="#"
                  >
              ...
              {selectedStockForChart && (
                <div className="candle-chart-container">
                  <h4>
                    ...
                    {loadingChart && (
                      <span className="text-muted ml-2">Загрузка...</span>
                    )}
                  </h4>
            ...
            <h2>
              ...
              >
                ...
                <button
                  type="button"
                  className="btn btn-sm btn-outline-secondary"
                  onClick={() => setActiveTab('Криптовалюта')}
                >
                  Криптовалюта
                </button>
                <button
              ...
              <table className="stock-table">
                {activeTab === 'Акции' && (
                  <StockTable
                    ...
                    openChartInNewWindow={handleOpenInNewWindow}
                  />
                )}
                {activeTab === 'Фьючерсы' && (
                  <FuturesTable
