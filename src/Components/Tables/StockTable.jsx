    ...
    <>
      ...
      <tbody style={{ textAlign: 'center' }}>
        {stock
          .filter(
            (item) =>
              selectedTradeMode === tradeModes[0] ||
              item.BOARDID === selectedTradeMode
          )
          .filter(
            (item) =>
              selectedListLevel === listLevel[0] ||
              item.LISTLEVEL === selectedListLevel
          )
          .map((item) => (
            <tr
              ...
              className={
                selectedStock?.SECID === item.SECID ? 'table-primary' : ''
              }
            >
              <td
              ...
              <td className="text-center ml-10">
                <span
                  style={{
                    backgroundColor:
                      item.BOARDID === 'TQBR' ? '#e6f0ff' : '#fff0e6',
              ...
              <td className="text-center price-cell ml-10">
                {item.LAST?.toFixed(2)}
              </td>
              ...
              <td
                className={
                  item.LASTCHANGE >= 0 ? 'positive-change' : 'negative-change'
                }
              >
                {item.LASTCHANGE >= 0 ? '+' : ''}
                {item.LASTCHANGE}%
              ...
              <td className="text-center">
                {item.HIGH && item.LOW
                  ? (((item.HIGH - item.LOW) / item.PREVPRICE) * 100).toFixed(
                      2
                    ) + '%'
                  : 'N/A'}
              </td>
